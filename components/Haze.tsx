'use client'

import { useEffect, useRef } from 'react'

/**
 * HAZE — o fundo das seções que não têm foto.
 *
 * Não é "partículas": é haze. No mundo do cliente, a fumaça é o que
 * faz o feixe aparecer — sem ela o moving head não desenha nada no ar.
 * Por isso o campo é denso embaixo e rarefeito em cima, e as partículas
 * puxam para o âmbar perto da base: é poeira pegando luz de palco.
 *
 * Adaptado de melhorias/back.md, com quatro correções:
 *  1. o rAF do original nunca era cancelado — vazamento no unmount;
 *  2. o `noise` era recriado a cada render E estava nas deps do
 *     useEffect, o que remontava o efeito em loop;
 *  3. `particleSize` como objeto nas deps causava o mesmo;
 *  4. sem porta de prefers-reduced-motion nem pausa fora da tela.
 *
 * Pausa quando sai do viewport e quando a aba perde foco.
 */

function criarRuido() {
  const perm = [
    151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,
    8,99,37,240,21,10,23,190,6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,
    35,11,32,57,177,33,88,237,149,56,87,174,20,125,136,171,168,68,175,74,165,71,
    134,139,48,27,166,77,146,158,231,83,111,229,122,60,211,133,230,220,105,92,41,
    55,46,245,40,244,102,143,54,65,25,63,161,1,216,80,73,209,76,132,187,208,89,18,
    169,200,196,135,130,116,188,159,86,164,100,109,198,173,186,3,64,52,217,226,250,
    124,123,5,202,38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,182,189,
    28,42,223,183,170,213,119,248,152,2,44,154,163,70,221,153,101,155,167,43,172,9,
    129,22,39,253,19,98,108,110,79,113,224,232,178,185,112,104,218,246,97,228,251,
    34,242,193,238,210,144,12,191,179,162,241,81,51,145,235,249,14,239,107,49,192,
    214,31,181,199,106,157,184,84,204,176,115,121,50,45,127,4,150,254,138,236,205,
    93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180,
  ]
  const p = new Array<number>(512)
  for (let i = 0; i < 256; i++) p[256 + i] = p[i] = perm[i]

  const fade = (t: number) => t * t * t * (t * (t * 6 - 15) + 10)
  const lerp = (t: number, a: number, b: number) => a + t * (b - a)
  const grad = (h: number, x: number, y: number, z: number) => {
    const g = h & 15
    const u = g < 8 ? x : y
    const v = g < 4 ? y : g === 12 || g === 14 ? x : z
    return ((g & 1) === 0 ? u : -u) + ((g & 2) === 0 ? v : -v)
  }

  return (x: number, y: number, z: number) => {
    const X = Math.floor(x) & 255, Y = Math.floor(y) & 255, Z = Math.floor(z) & 255
    x -= Math.floor(x); y -= Math.floor(y); z -= Math.floor(z)
    const u = fade(x), v = fade(y), w = fade(z)
    const A = p[X] + Y, AA = p[A] + Z, AB = p[A + 1] + Z
    const B = p[X + 1] + Y, BA = p[B] + Z, BB = p[B + 1] + Z
    return lerp(w,
      lerp(v, lerp(u, grad(p[AA], x, y, z), grad(p[BA], x - 1, y, z)),
              lerp(u, grad(p[AB], x, y - 1, z), grad(p[BB], x - 1, y - 1, z))),
      lerp(v, lerp(u, grad(p[AA + 1], x, y, z - 1), grad(p[BA + 1], x - 1, y, z - 1)),
              lerp(u, grad(p[AB + 1], x, y - 1, z - 1), grad(p[BB + 1], x - 1, y - 1, z - 1))))
  }
}

export function Haze({ densidade = 900 }: { densidade?: number }) {
  const ref = useRef<HTMLCanvasElement>(null)

  useEffect(() => {
    const canvas = ref.current
    if (!canvas) return
    if (matchMedia('(prefers-reduced-motion: reduce)').matches) return

    const ctx = canvas.getContext('2d', { alpha: true })
    if (!ctx) return

    const ruido = criarRuido()
    // teto de DPR: em telefone com DPR 3 o custo triplica sem ganho visível
    const dpr = Math.min(window.devicePixelRatio || 1, 1.5)
    let w = 0, h = 0

    const medir = () => {
      const r = canvas.getBoundingClientRect()
      w = r.width; h = r.height
      canvas.width = Math.round(w * dpr)
      canvas.height = Math.round(h * dpr)
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0)
    }
    medir()

    // menos partícula em tela pequena — o público chega por 4G
    const n = w < 640 ? Math.round(densidade * 0.45) : densidade
    const parts = Array.from({ length: n }, () => ({
      x: Math.random() * w,
      y: Math.random() * h,
      s: Math.random() * 1.5 + 0.4,
      vida: Math.random() * 100,
      max: 100 + Math.random() * 60,
    }))

    let id = 0
    let naTela = true      // esta no viewport?
    let comFoco = true     // a aba esta em primeiro plano?
    const rodando = () => naTela && comFoco

    const quadro = () => {
      // rastro: limpa com preto translúcido em vez de apagar
      ctx.fillStyle = 'rgba(9, 9, 11, 0.14)'
      ctx.fillRect(0, 0, w, h)

      const t = performance.now() * 0.0001
      for (const p of parts) {
        p.vida += 1
        if (p.vida > p.max) {
          p.vida = 0
          p.x = Math.random() * w
          p.y = Math.random() * h
        }
        const a = ruido(p.x * 0.003, p.y * 0.003, t) * Math.PI * 4
        p.x += Math.cos(a) * 1.6
        p.y += Math.sin(a) * 1.6
        if (p.x < 0) p.x = w
        if (p.x > w) p.x = 0
        if (p.y < 0) p.y = h
        if (p.y > h) p.y = 0

        // haze pega mais luz perto do chão: mais denso e mais âmbar embaixo
        const baixo = p.y / h
        const op = Math.sin((p.vida / p.max) * Math.PI) * (0.05 + baixo * 0.13)
        ctx.fillStyle = `rgba(255, 163, 0, ${op * baixo})`
        ctx.beginPath()
        ctx.arc(p.x, p.y, p.s, 0, Math.PI * 2)
        ctx.fill()
        ctx.fillStyle = `rgba(236, 237, 239, ${op * (1 - baixo) * 0.7})`
        ctx.beginPath()
        ctx.arc(p.x, p.y, p.s * 0.8, 0, Math.PI * 2)
        ctx.fill()
      }
      if (rodando()) id = requestAnimationFrame(quadro)
    }

    // só desenha quando está na tela e a aba está em foco
    /* Duas condicoes INDEPENDENTES. Antes elas estavam colapsadas numa
       variavel so, entao ao voltar da aba `visivel` ja era false e a
       expressao nunca conseguia religar: o haze congelava. */
    const religa = () => {
      cancelAnimationFrame(id)
      if (rodando()) id = requestAnimationFrame(quadro)
    }
    const io = new IntersectionObserver(([e]) => {
      naTela = e.isIntersecting
      religa()
    }, { threshold: 0 })
    io.observe(canvas)

    const onVis = () => { comFoco = !document.hidden; religa() }
    document.addEventListener('visibilitychange', onVis)

    const ro = new ResizeObserver(medir)
    ro.observe(canvas)

    id = requestAnimationFrame(quadro)

    return () => {
      cancelAnimationFrame(id)   // o original nunca fazia isto
      io.disconnect()
      ro.disconnect()
      document.removeEventListener('visibilitychange', onVis)
    }
  }, [densidade])

  return (
    <canvas ref={ref} aria-hidden
            className="pointer-events-none absolute inset-0 -z-10 h-full w-full" />
  )
}
